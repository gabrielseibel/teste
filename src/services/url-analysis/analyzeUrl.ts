import { ALL_KNOWN_DOMAINS } from './knownDomains';
...
function findLookalike(hostname: string): { domain: string; distance: number } | null {
  let best: { domain: string; distance: number } | null = null;
  for (const known of ALL_KNOWN_DOMAINS) {
...
export function analyzeUrl(rawUrl: string): UrlAnalysis {
  ...
  const lookalike = findLookalike(hostname);
